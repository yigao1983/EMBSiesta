...
!
 if(Node.eq.0) write(6,'(/a/)') 'siesta_evolut: Evolution type: wfk' 
!
 stagenfr = 'td'
! 
...
!
 if(associated(zwfk))     deallocate(zwfk)      ; nullify(zwfk)
 allocate(    zwfk(nuotot,max(nuolococc,1),nspin))
 if(associated(zDaux_old)) deallocate(zwfk_old) ; nullify(zwfk_old)
 allocate(zwfk_old(nuotot,max(nuolococc,1),nspin))
!
 if(nuolococc.gt.0) zwfk(:,1:nuolococc,:) = dcmplx(wfk(:,1:nuolococc,:))
!
 itim = 0
 call zpsi2daux(zwfk, Daux)
...
!
    zwfk_old(:,:,:) = zwfk(:,:,:)
    ! TDDFT
    if(.not.pseudoatm) call magnus_wfk(itim, dtim, zwfk_old, zwfk)
    ! Near-field
    if(.not.pseudonfr) then
...
!
 end subroutine siesta_evolut_wfk
